...
class UploadFailedEvent(FailedEvent, kw_only=True, frozen=True):
    """
    Evento emitido quando uma operação de upload falha.

    Este evento faz parte do fluxo EDA (Event-Driven Architecture) do Seisbai
    e representa a ocorrência de um erro durante a tentativa de enviar um arquivo.
    ...
    **somente nomeados** (`kw_only=True`), aumentando clareza e segurança.
    ...
    -------
    FailedEvent
        Evento-base que representa operações concluídas com falha no fluxo EDA.

    ---
    Atributos
    ---------
    file_name : str
        Nome do arquivo cujo upload falhou.

    ---
    Uso
    ---
    O handler responsável pelo upload deve emitir este evento quando ocorrer
    ...
    Exemplos de uso:
        - informar o frontend de que o upload não pôde ser concluído
