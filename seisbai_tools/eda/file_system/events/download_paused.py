...
class DownloadPausedEvent(PausedEvent, kw_only=True, frozen=True):
    """
    Evento emitido quando uma operação de download é pausada.

    Este evento integra o fluxo EDA (Event-Driven Architecture) do Seisbai
    e representa a interrupção temporária de um download em andamento. Ele
    ...
    ---------
    file_name : str
        Nome do arquivo cujo download foi pausado.

    ---
    Uso
    ---
    Este evento deve ser emitido pelo handler responsável quando o download
